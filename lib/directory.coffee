...
module.exports =
class Directory
  constructor: ({@allowedFiles, @name, fullPath, @symlink, @expansionState, @isRoot, @ignoredPatterns, @useSyncFS, @stats}) ->
  ...
  getEntries: ->
    ...
    for name in names
      ...
      #continue if !name.includes("json")
      ...
      if stat.isDirectory?()
        if(!@allowedFiles.isDirAllowed(fullPath)) then  continue
        if @entries.hasOwnProperty(name)
          ...
          directories.push(new Directory({@allowedFiles, name, fullPath, symlink, expansionState, @ignoredPatterns, @useSyncFS, stats: statFlat}))
      else if stat.isFile?()
        if @entries.hasOwnProperty(name)
          # push a placeholder since this entry already exists but this helps
          # track the insertion index for the created views
          ...
          #filter here
          if(@allowedFiles.isFileAllowed(fullPath))
